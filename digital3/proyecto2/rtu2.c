...
void receive_UDP(void *ptr)
{
    ...
    while(1)
    {
        ...
        if(strstr(msg, "reconnect") != NULL) //Si es reconectar el historiador se está intentando volver a conectar
        {
            client = from;
        }

        if(strstr(msg, "RTU2") != NULL) //Chequear si el mensaje va para la RTU2 o no
        {
            if(strstr(msg, "led1_on") != NULL) //Si el mensaje contiene led1_on...
            {
                if(digitalRead(LED1) == LOW) //Si la led1 estaba apagada encenderla
                {
                    ...
                    //Obtener reporte
					sprintf(reporte[events], "RTU2 REPORT AT EVENT: LED1 turned on at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
					events++;
                    sem_post(&semaphore1); //Devolver semaphore
            ...
            else if(strstr(msg, "led1_off") != NULL) //Si el mensaje contiene led1_off...
            {
                if(digitalRead(LED1) == HIGH) //Si la led1 estaba encendida apagarla
                {
                    ...
                    //Obtener reporte
					sprintf(reporte[events], "RTU2 REPORT AT EVENT: LED1 turned off at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
					events++;
                    sem_post(&semaphore1); //Devolver semaphore
                    usleep(1000);
                }
            ...
            else if(strstr(msg, "led2_on") != NULL) //Si el mensaje contiene led2_on...
            {
                if(digitalRead(LED2) == LOW) //Si la led1 estaba apagada encenderla
                {
                    ...
                    //Obtener reporte
					sprintf(reporte[events], "RTU2 REPORT AT EVENT: LED2 turned on at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
					events++;
                    sem_post(&semaphore1); //Devolver semaphore
                    usleep(1000); //Delay mínimo
                }
            ...
            else if(strstr(msg, "led2_off") != NULL) //Si el mensaje contiene led2_off...
            {
                if(digitalRead(LED2) == HIGH) //Si la led2 estaba encendida apagarla
                {
                    ...
                    //Obtener reporte
					sprintf(reporte[events], "RTU2 REPORT AT EVENT: LED2 turned off at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
					events++;
                    sem_post(&semaphore1); //Devolver semaphore
                    usleep(1000); //Delay mínimo
                }
            ...
            else if(strstr(msg, "led_IoT") != NULL) //Si el mensaje contiene led1_on...
            {
                ...
                if(digitalRead(LED_IOT) == HIGH) //Si la led1 estaba apagada encenderla
                {
                    ...
                    sem_wait(&semaphore1); //Tomar semaphore
                    //sprintf(stringArray[events], "RTU2: Led IoT ON at %s\n", timeString); //Guardar en eventos
                    //Obtener reporte
					sprintf(reporte[events], "RTU2 REPORT AT EVENT: LED IoT turned on at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
					events++;
                    sem_post(&semaphore1); //Devolver semaphore
                    usleep(1000); //Delat mínimo
                }
                ...
                else
                {
                    ...
                    sem_wait(&semaphore1); //Tomar semaphore
                    //sprintf(stringArray[events], "RTU2: Led IoT OFF at %s\n", timeString); //Guardar en eventos
                    //Obtener reporte
					sprintf(reporte[events], "RTU2 REPORT AT EVENT: LED IoT turned off at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
					events++;
                    sem_post(&semaphore1); //Devolver semaphore
                    usleep(1000); //Delat mínimo
                }
...
void get_ADC(void *ptr)
{
    ...
    while(1)
    {
        ...
        if ((adc_voltage <= 2.5) && (adc_voltage >= 0.5) && (normal_flag == 0)) //Si se está en operación normal...
        {
			...
			fflush(stdout);
            sem_wait(&semaphore1); //Tomar semaphore
            //sprintf(stringArray[events], "RTU2: Normal Operation on line at %s\n", timeString); //Guardar en eventos
            //Obtener reporte
			sprintf(reporte[events], "RTU2 REPORT AT EVENT: Normal Operation on line at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
            events++;
            sem_post(&semaphore1); //Devolver semaphore
            usleep(1000); //Delay mínimo
            usleep(500000); 
        ...
        else if((adc_voltage > 2.45) && (overcharge_flag == 0)) //Si el adc es mayor a 2.5V
        {
            ...
            sem_wait(&semaphore1); //Tomar semaphore
            //sprintf(stringArray[events], "RTU2: Overcharge on line at %s\n", timeString); //Guardar en eventos
            //Obtener reporte
			sprintf(reporte[events], "RTU2 REPORT AT EVENT: Overcharge on line at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
            events++;
            sem_post(&semaphore1); //Devolver semaphore
            usleep(1000); //Delay mínimo
            usleep(500000); 
        ...
        else if((adc_voltage < 0.45) && (undercharge_flag == 0))
        {
            ...
            sem_wait(&semaphore1); //Tomar semaphore
            //sprintf(stringArray[events], "RTU2: Undercharge on line at %s\n", timeString); //Guardar en eventos
            //Obtener reporte
			sprintf(reporte[events], "RTU2 REPORT AT EVENT: Undercharge on line at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
            events++;
            sem_post(&semaphore1); //Devolver semaphore
            usleep(1000); //Delay mínimo 
            usleep(500000);
...
void boton1(void)
{
    ...
    if(digitalRead(BUTTON1) == LOW) // Confirma que sigue presionado
    {
        ...
        {
            printf("RTU2: Button 1 pressed\n");
            fflush(stdout);
            sem_wait(&semaphore1);
            //sprintf(stringArray[events], "RTU2: Button 1 pressed at: %s\n", timeString);
            //Obtener reporte
			sprintf(reporte[events], "RTU2 REPORT AT EVENT: Button 1 pressed at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
            events++;
            sem_post(&semaphore1);
        }
    }
...
void boton2(void)
{
    ...
    if(digitalRead(BUTTON2) == LOW)
    {
        ...
        if(digitalRead(BUTTON2) == LOW)
        {
            printf("RTU2: Button 2 pressed\n");
            fflush(stdout);
            sem_wait(&semaphore1);
            //sprintf(stringArray[events], "RTU2: Button 2 pressed at: %s\n", timeString);
            //Obtener reporte
			sprintf(reporte[events], "RTU2 REPORT AT EVENT: Button 2 pressed at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
            events++;
            sem_post(&semaphore1);
        }
    }
    ...
    if(current_state == digitalRead(DIP1)) // Confirmación tras debounce
    {
        if(current_state != dip1_previous_state) // solo si realmente cambió
        {
            ...
            if(current_state == LOW)
            {
                ...
                printf("RTU2: Switch 1 changed state to OFF\n");
            }
            else
            {
                strcpy(dip1_state, "ON");
                printf("RTU2: Switch 1 changed state to ON\n");
            }

            fflush(stdout);
            sem_wait(&semaphore1);
            //sprintf(stringArray[events], "RTU2: Switch 1 changed state to %s at: %s\n", dip1_state, timeString);
            //Obtener reporte
			sprintf(reporte[events], "RTU2 REPORT AT EVENT: Switch 1 changed state at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
            events++;
            sem_post(&semaphore1);
        }
    }
    ...
    if(current_state == digitalRead(DIP2)) // Confirmación tras debounce
    {
        if(current_state != dip2_previous_state) // solo si realmente cambió
        {
            ...
            if(current_state == LOW)
            {
                ...
                printf("RTU2: Switch 2 changed state to OFF\n");
            }
            else
            {
                strcpy(dip2_state, "ON");
                printf("RTU2: Switch 2 changed state to ON\n");
            }

            fflush(stdout);
            sem_wait(&semaphore1);
            //sprintf(stringArray[events], "RTU2: Switch 2 changed state to %s at: %s\n", dip2_state, timeString);
			
			//Obtener reporte
			sprintf(reporte[events], "RTU2 REPORT AT EVENT: Switch 2 changed state at %s\nButton 1: %s, Button 2: %s,\nInterruptor 1: %s, Interruptor 2: %s,\nLed 1: %s, Led 2: %s,\nIot Led: %s,\nVoltage on line 1: %.2f\n", timeString, button1_state, button2_state, dip1_state, dip2_state, led1_state, led2_state, lediot_state, adc_voltage);
            events++;
            sem_post(&semaphore1);
        }
    }
